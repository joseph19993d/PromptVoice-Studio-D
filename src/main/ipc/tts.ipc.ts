...
export function registerTTSHandlers(ttsManager: TTSManager): void {
  ipcMain.handle(
    IPC_CHANNELS.TTS_GENERATE,
    async (_event, text: string, options?: TTSOptions) => {
      try {
        const audioBuffer = await ttsManager.generate(text, options)
  ...
  ipcMain.handle(IPC_CHANNELS.TTS_VOICES, async () => {
    try {
      return { success: true, data: await ttsManager.listVoices() }
    } catch (error) {
      return { success: false, error: (error as Error).message }
    }
  })
